package com.wemote.sdk.wechat.support.http.client;
...
public class XMLResponseHandler {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(XMLResponseHandler.class);
            ...
            ResponseHandler<T> responseHandler = new ResponseHandler<T>() {
                ...
                public T handleResponse(HttpResponse response)
                    ...
                    if (status.getStatusCode() >= HttpStatus.SC_OK && status.getStatusCode() < HttpStatus.SC_MULTIPLE_CHOICES) {
                        ...
                        T t = XMLUtils.toBean(new String(str.getBytes("iso-8859-1"), "utf-8"), clazz);

                        if (t == null) {
                            LOG.error("XML parse error:{}", str);
                            // ignore;
                        }
